...
const Predictor = () => {
    ...
    const handleKeyDown = (e, currentIndex, steprefs) => {
        if (e.key === 'Enter') {
            e.preventDefault();
...
=============================================`;
    ...
    const InputField = ({ label, name, type = "number", icon: Icon, inputRef, onKeyDown }) => (
                ...
                {step === 1 && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <InputField label="Work Hours / Day" name="avg_work_hours" inputRef={workHoursRef} onKeyDown={(e) => handleKeyDown(e, 0, step1Refs)} />
                            <InputField label="Rest Hours / Day" name="avg_rest_hours" inputRef={restHoursRef} onKeyDown={(e) => handleKeyDown(e, 1, step1Refs)} />
                            <InputField label="Sleep Hours / Day" name="avg_sleep_hours" inputRef={sleepHoursRef} onKeyDown={(e) => handleKeyDown(e, 2, step1Refs)} />
                            <InputField label="Exercise / Day" name="avg_exercise_hours" inputRef={exerciseHoursRef} onKeyDown={(e) => handleKeyDown(e, 3, step1Refs)} />
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-widest ml-1">Gender</label>
                                <select ref={genderRef} name="gender" value={formData.gender} onChange={handleInputChange} onKeyDown={(e) => handleKeyDown(e, 4, step1Refs)} className="w-full p-3 bg-slate-900/50 border border-slate-700/50 rounded-xl text-white outline-none focus:border-blue-500 text-sm hover:bg-slate-800">
                                    <option value={1} className="bg-slate-900">Male</option>
                                    <option value={0} className="bg-slate-900">Female</option>
                                </select>
                            </div>
                            <InputField label="Occupation Type (ID)" name="occupation_type" inputRef={occupationTypeRef} onKeyDown={(e) => handleKeyDown(e, 5, step1Refs)} />
                        </div>
                ...
                {step === 2 && (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-700">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <InputField label="Age" name="age" icon={User} inputRef={ageRef} onKeyDown={(e) => handleKeyDown(e, 0, step2Refs)} />
                            <InputField label="Stress Level (1-10)" name="stress_level_health" inputRef={stressRef} onKeyDown={(e) => handleKeyDown(e, 1, step2Refs)} />
                            <InputField label="Sleep Quality (1-10)" name="quality_of_sleep" inputRef={sleepQualityRef} onKeyDown={(e) => handleKeyDown(e, 2, step2Refs)} />
                            <InputField label="Daily Steps" name="daily_steps" inputRef={dailyStepsRef} onKeyDown={(e) => handleKeyDown(e, 3, step2Refs)} />
                            <InputField label="Heart Rate (BPM)" name="heart_rate" inputRef={heartRateRef} onKeyDown={(e) => handleKeyDown(e, 4, step2Refs)} />
                            <InputField label="Systolic BP" name="systolic_bp" inputRef={systolicRef} onKeyDown={(e) => handleKeyDown(e, 5, step2Refs)} />
                            <InputField label="Diastolic BP" name="diastolic_bp" inputRef={diastolicRef} onKeyDown={(e) => handleKeyDown(e, 6, step2Refs)} />
                            <InputField label="BMI (0,1,2)" name="bmi_category" inputRef={bmiRef} onKeyDown={(e) => handleKeyDown(e, 7, step2Refs)} />
                            <InputField label="Physical Activity" name="physical_activity_level" inputRef={physicalActivityRef} onKeyDown={(e) => handleKeyDown(e, 8, step2Refs)} />
                            <InputField label="Sleep Duration" name="sleep_duration" inputRef={sleepDurationRef} onKeyDown={(e) => handleKeyDown(e, 9, step2Refs)} />
                            <InputField label="Occupation (ID)" name="occupation" inputRef={occupationRef} onKeyDown={(e) => handleKeyDown(e, 10, step2Refs)} />
                        </div>
                ...
                {step === 3 && (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                            <InputField label="Annual Income" name="annual_income" icon={Landmark} inputRef={incomeRef} onKeyDown={(e) => handleKeyDown(e, 0, step3Refs)} />
                            <InputField label="Total Savings" name="savings_assets" icon={TrendingUp} inputRef={savingsRef} onKeyDown={(e) => handleKeyDown(e, 1, step3Refs)} />
                            <InputField label="Credit Score" name="credit_score" icon={CreditCard} inputRef={creditScoreRef} onKeyDown={(e) => handleKeyDown(e, 2, step3Refs)} />
                            <InputField label="Years Employed" name="years_employed" inputRef={yearsEmployedRef} onKeyDown={(e) => handleKeyDown(e, 3, step3Refs)} />
                            <InputField label="Current Debt" name="current_debt" inputRef={debtRef} onKeyDown={(e) => handleKeyDown(e, 4, step3Refs)} />
                            <InputField label="Equity Market (%)" name="Equity_Market" inputRef={equityRef} onKeyDown={(e) => handleKeyDown(e, 5, step3Refs)} />
                            <InputField label="Fixed Deposits" name="Fixed_Deposits" inputRef={fixedDepositsRef} onKeyDown={(e) => handleKeyDown(e, 6, step3Refs)} />

                            <div className="flex flex-col gap-1.5">
                                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-widest ml-1">Stock Market</label>
                                <select ref={stockMarketRef} name="stock_market" value={formData.stock_market} onChange={handleInputChange} onKeyDown={(e) => handleKeyDown(e, 7, step3Refs)} className="w-full p-3 bg-slate-900/50 border border-slate-700/50 rounded-xl text-white outline-none focus:border-indigo-500 text-sm hover:bg-slate-800">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <select ref={investmentRef} name="investment_avenues" value={formData.investment_avenues} onChange={handleInputChange} onKeyDown={(e) => handleKeyDown(e, 8, step3Refs)} className="w-full p-3 bg-slate-900/50 border border-slate-700/50 rounded-xl text-white outline-none focus:border-indigo-500 text-sm hover:bg-slate-800">
                                    <option value="No" className="bg-slate-900">No</option>
                                    <option value="Yes" className="bg-slate-900">Yes</option>
                                </select>
                            </div>
                            <div className="flex flex-col gap-1.5 col-span-1 md:col-span-2">
                                ...
                                <input ref={occupationStatusRef} type="text" name="occupation_status" value={formData.occupation_status} onChange={handleInputChange} onKeyDown={(e) => handleKeyDown(e, 9, step3Refs)} className="w-full p-3 bg-slate-900/50 border border-slate-700/50 rounded-xl text-white outline-none focus:border-indigo-500 text-sm hover:bg-slate-800" placeholder="e.g. Self-Employed" />
                            </div>
                        </div>
